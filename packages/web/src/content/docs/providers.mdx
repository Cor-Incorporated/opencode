...
```json title="opencode.json" "cor-local" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "cor-local": {
...
This provider is pre-wired in `packages/guardrails/managed/opencode.json` and `packages/guardrails/profile/opencode.json` with three model ids (`glm53-flash`, `deepseek-v4-flash-0731`, `qwen3.8-27b`), and does not change the default model — select it explicitly with `--model cor-local/glm53-flash`. See `packages/guardrails/README.md` for the offline usage recipe and cold-start notes.
