...
test("keeps the locked edit schema, semantics docstring, and deferred TODOs visible", async () => {
  ...
  const namedProjectReferenceConstraint =
    "Named project references are read-oriented and deliberately are not accepted by mutation tools."
  const pathProperty = schema.properties?.path as { readonly description?: string } | undefined
