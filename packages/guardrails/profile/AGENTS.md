...
- `cor-local` is an opt-in, fully offline lane routed through a local `llama-server` router (Mac Studio, default `http://127.0.0.1:18082/v1`; `options.baseURL`/`options.apiKey` resolve from `COR_LOCAL_BASE_URL`/`COR_LOCAL_API_KEY`, defaulted by `bin/opencode-guardrails` — the Node entry point run on every invocation — when unset). It never changes the default model — select it explicitly with `--model cor-local/<id>` (`deepseek-v4-flash-0731`, `glm53-flash`, or `qwen3.8-27b`) when the task must run air-gapped. See `packages/guardrails/README.md`'s "cor-local (ローカル lane)" section for the env vars, MacBook setup, offline env recipe, and cold-start behavior.
- Z.AI lanes: Coding Plan subscriptions cover the `zai-coding-plan` endpoint only; the `zai` platform lane is not admitted by default (removed 2026-09-11 — Coding Plan accounts have no platform balance). `bin/opencode-guardrails` bridges a stored `zai`/`zai-coding-plan` API key into `ZHIPU_API_KEY` when unset (same entry-point defaulting as cor-local), and `guardrail.ts`'s `dropKeylessCatalogLanes` keeps a re-declared lane invisible on machines without any stored key. See `packages/guardrails/README.md`'s "ZHIPU_API_KEY (z.ai lane credential bridge)" section for re-enabling the platform lane.
