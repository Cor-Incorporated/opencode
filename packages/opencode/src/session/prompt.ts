...
import { stripThinkTags } from "../util/format"
...
import { runHooks, safeToolInput, safeToolOutput, type HookEnv, type HookResult } from "../hook"
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
                ...
                Effect.gen(function* () {
                  ...
                  const postHookResult = yield* Effect.promise(() =>
                    runHooks(hookCfg.hooks?.PostToolUse, item.id, {
                      ...
                      OPENCODE_TOOL_OUTPUT: safeToolOutput(output.output),
                  ...
                  const merged = { ...output, output: finalOutput }
                  if (options.abortSignal?.aborted) {
                    yield* input.processor.completeToolCall(options.toolCallId, merged)
                  }
                  return merged
                }),
              )
              ...
              Effect.gen(function* () {
                ...
                const mcpPostResult = yield* Effect.promise(() =>
                  runHooks(mcpHookCfg.hooks?.PostToolUse, key, {
                    ...mcpHookEnv,
                    OPENCODE_HOOK_EVENT: "PostToolUse",
                    OPENCODE_TOOL_OUTPUT: safeToolOutput(result.content.map((c: { type: string; text?: string }) => c.type === "text" ? c.text : `[${c.type}]`).join("\n")),
                  }).catch((): HookResult => ({ action: "pass" })),
                )
      ...
      const loop: (input: z.infer<typeof LoopInput>) => Effect.Effect<MessageV2.WithParts> = Effect.fn(
        ...
        return yield* state.ensureRunning(input.sessionID, latestAssistant(input.sessionID), runLoop(input.sessionID))
      })

      const shell: (input: ShellInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.shell")(
        function* (input: ShellInput) {
          return yield* state.startShell(input.sessionID, latestAssistant(input.sessionID), shellImpl(input))
        },
      )
  ...
  const defaultLayer = Layer.suspend(() =>
    layer.pipe(
      ...
      Layer.provide(Layer.merge(Bus.layer, Layer.merge(CrossSpawnSpawner.defaultLayer, Config.defaultLayer))),
    ),
  )
