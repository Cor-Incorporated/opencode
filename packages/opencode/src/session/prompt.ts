  ...
  Effect.gen(function* () {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      const permissions = Object.entries(input.tools ?? {}).map(
        ([permission, enabled]) =>
          ({ permission, action: enabled ? "allow" : "deny", pattern: "*" }) satisfies PermissionV1.Rule,
      )
      if (permissions.length > 0) {
        const toolPermissions = new Set(permissions.map((rule) => rule.permission))
        const promptToolPermissions = session.parentID ? toolPermissions : promptToolPermissionIDs(toolPermissions)
        session.permission = session.parentID
          ? Permission.merge(
              (session.permission ?? []).filter((rule) => !toolPermissions.has(rule.permission)),
              permissions,
            )
          : Permission.merge(
              (session.permission ?? []).filter((rule) => !promptToolPermissions.has(rule.permission)),
              permissions,
            )
...
const PROMPT_TOOL_PERMISSION_IDS = [
  "apply_patch",
  "bash",
  "edit",
  "glob",
...
function promptToolPermissionIDs(current: ReadonlySet<string>) {
  return new Set([...PROMPT_TOOL_PERMISSION_IDS, ...current])
}
