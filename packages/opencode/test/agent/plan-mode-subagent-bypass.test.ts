...
it.instance("subagent worker baseline applies when no parent agent ceiling is provided", () =>
  Effect.gen(function* () {
...
it.instance("parent agent edit denial remains effective for spawned subagents", () =>
  Effect.gen(function* () {
    const planAgent = yield* Agent.use.get("plan")
    const generalAgent = yield* Agent.use.get("general")
    expect(planAgent).toBeDefined()
...
it.instance(
  "custom subagent can enable edits when no parent agent ceiling is provided",
  () =>
    Effect.gen(function* () {
      const planAgent = yield* Agent.use.get("plan")
...
it.instance(
  "custom subagent cannot override parent agent edit denial",
  () =>
    Effect.gen(function* () {
      const planAgent = yield* Agent.use.get("plan")
      const my = yield* Agent.use.get("my_subagent")
