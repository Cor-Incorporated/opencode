...
it.instance("subtask child inherits parent session external_directory allow", () =>
  Effect.gen(function* () {
    ...
    expect(rules).toContainEqual({ permission: "external_directory", pattern: "/tmp/allowed/*", action: "allow" })
  ...
  noLLMServer.instance("prompt tools replace previous prompt tool rules without dropping session scoped rules", () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({
