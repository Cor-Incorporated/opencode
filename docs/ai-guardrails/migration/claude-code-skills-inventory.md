...
This inventory is a gating artifact, not a loose note. If a future implementation cannot point to a row or rule in this document, that implementation needs an explicit ADR or inventory update first.

## Source canon

The inventory is derived from these sources:

- `claude-code-skills` README
- `claude-code-skills` epic `#130`
- `claude-code-skills` ADRs `001` to `004`
